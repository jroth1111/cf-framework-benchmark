...
const footerHtml = 'React + Vite variant • <span class="kbd">/chart</span> is SPA-like, blog is SSG pages, stays are multi-page routes.';

// Core pages
writeFile(path.join(pagesDir, "index.html"), html({ docTitle: "CF Bench — React + Vite", entry: "home" }));
writeFile(path.join(pagesDir, "stays", "index.html"), html({ docTitle: "Stays — React + Vite", entry: "stays" }));
writeFile(path.join(pagesDir, "chart", "index.html"), html({ docTitle: "Chart — React + Vite", entry: "chart" }));

// Listing detail pages
for (const l of listings) {
  writeFile(
    path.join(pagesDir, "stays", l.id, "index.html"),
    html({ docTitle: `${l.title} — React + Vite`, entry: "stay", props: { id: l.id } })
  );
}
...
writeFile(
  ...
  html({ docTitle: "Blog — React + Vite", bodyHtml: blogIndexBody, includeCss: true })
);
...
for (const p of blogPosts) {
  ...
  writeFile(
    ...
    html({ docTitle: `${p.title} — React + Vite`, bodyHtml: postBody, includeCss: true })
  );
}
