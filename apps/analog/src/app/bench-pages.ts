...
function updateAnalogChartCoreMetrics(updates: Record<string, unknown>) {
  const root = ensureBenchRoot();
  root.chartCore = {
    ...(root.chartCore || {}),
    ...updates,
...
function getAnalogChartFetchOptions(): RequestInit | undefined {
  const mode = (window as typeof window & { __CF_BENCH_CONFIG__?: { chartCache?: string } }).__CF_BENCH_CONFIG__?.chartCache;
  return mode === 'no-store' ? { cache: 'no-store' } : undefined;
}

function startAnalogChartSwitch() {
  const root = ensureBenchRoot();
  root.chart = {
    ...(root.chart || {}),
    ready: false,
...
function markAnalogChartReady(symbol: string, timeframe: string) {
  const root = ensureBenchRoot();
  const switchStartTs = root.chart?.switchStartTs || performance.now();
...
function markAnalogChartError(error: Error | string) {
  const root = ensureBenchRoot();
  root.chart = {
    ...(root.chart || {}),
    ready: true,
...
export class ChartPageComponent implements OnDestroy {
  ...
  private async refreshChart() {
    ...
    try {
      ...
      markAnalogChartError(error instanceof Error ? error : String(error));
      this.status = 'error';
    }
  }
}
