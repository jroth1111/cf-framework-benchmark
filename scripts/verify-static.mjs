...
await run("test:contract-api-functional", "pnpm", ["test:contract-api-functional"]);
await run("test:contract-kind-invariants", "pnpm", ["test:contract-kind-invariants"]);
await run("test:bench-cache-classify", "pnpm", ["test:bench-cache-classify"]);
await run("test:single-framework-registry", "pnpm", ["test:single-framework-registry"]);
await run("test:cache-derivation", "pnpm", ["test:cache-derivation"]);
await run("test:rubric-coverage", "pnpm", ["test:rubric-coverage"]);
await run("test:hifi-suite-frameworks", "pnpm", ["test:hifi-suite-frameworks"]);
await run("test:matrix-schema", "pnpm", ["test:matrix-schema"]);
await run("check:hifi-frameworks", "pnpm", ["check:hifi-frameworks"]);
await run("test:framework-route-parity", "pnpm", ["test:framework-route-parity"]);
await run("test:next-patch-parity", "pnpm", ["test:next-patch-parity"]);
await run("test:resolve-sample-parity", "pnpm", ["test:resolve-sample-parity"]);
await run("test:rubric-metric-parity", "pnpm", ["test:rubric-metric-parity"]);
