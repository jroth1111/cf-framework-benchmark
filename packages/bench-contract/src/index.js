...
<html lang="en">
  <head>
    ...
    <script>
      (function () {
        ...
        if (!Number.isFinite(h.startMs)) h.startMs = performance.now();
      })();
    </script>
    ${extraHead}
  </head>
  <body>
    <header class="container nav">
      <a class="brand" href="/">CF Bench (${esc(framework)})</a>
    ...
    <main class="container">
      ${body}
      <div class="footer">${esc(framework)} benchmark route surface (Workers runtime).</div>
    </main>
    <script>
      (function () {
        var w = globalThis;
...
function staysPage(framework, input) {
  const url = toUrl(input);
  const page = parseIntParam(url.searchParams.get("page"), 1);
  const pageSize = parseIntParam(url.searchParams.get("pageSize"), 12);
...
function stayDetailPage(framework, id) {
  const listing = getListing(id);
  if (!listing) {
    return shell(
      framework,
...
function blogPage(framework) {
  ...
  return shell(
    framework,
    "Blog",
    `<h1 class="h1">Blog</h1><div class="grid cols-2">${cards}</div>`
  );
}

function blogPostPage(framework, slug) {
  const post = getPost(slug);
  if (!post) {
    return shell(framework, "Post not found", `<h1 class="h1">Post not found</h1>`);
  }
  return shell(
    framework,
    post.title,
        ...
        <div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center">
          <label class="pill">
            ...
            <select class="input" data-testid="symbol-select">
              ${chartSymbols.map((s) => `<option value="${esc(s)}">${esc(s)}</option>`).join("")}
            </select>
          </label>
          <label class="pill">
        ...
        (function () {
          ...
          fetch('/api/media?pageSize=30')
            ...
            .catch(function (err) {
              ...
              media.ready = true;
              media.error = true;
              media.errorMessage = err && err.message ? err.message : String(err);
            });
        })();
      </script>
    `
...
export function renderBenchmarkPage(framework, input) {
  const url = toUrl(input);
  const match = matchBenchmarkPage(url.pathname);
  if (!match) return null;

  switch (match.page) {
    case "home":
      return homePage(framework);
    case "stays":
      return staysPage(framework, url);
    case "stay":
      return stayDetailPage(framework, match.id);
    case "blog":
      return blogPage(framework);
    case "post":
      return blogPostPage(framework, match.slug);
    case "chart":
      return chartPage(framework);
    case "media":
      return mediaPage(framework);
    default:
      return null;
  }
}
...
export function handleBenchmarkRequest(framework, request) {
  const url = toUrl(request);
  if (request?.method && request.method.toUpperCase() !== "GET") return null;

  const api = handleContractApi(framework, url);
  if (api) return api;

  const html = renderBenchmarkPage(framework, url);
  if (!html) return null;
